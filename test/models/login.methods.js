...
export const loginToAdmin = async (invalid = 'valid') => {
  ...
  if (!baseUrl.includes('prokit.me')) {
    ...
    user = 'robert_davis';
    password = 'c2e16ac7cbeaa1f8';
  }
  await browser.url(baseUrl + testData.endpoints.adminEndpoint);
  if (await AdminDashboard.leftMenu.casinosTabInLeftMenu().isExisting() === false) {
    if (await AdminDashboard.login.emailInput().isExisting() === false) {
