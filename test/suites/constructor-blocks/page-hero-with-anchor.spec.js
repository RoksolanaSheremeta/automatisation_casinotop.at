import AdminDashboard from '../../pageobjects/admin-dashboard.selectors';
import { loginToAdmin } from '../../models/login.methods';
import { updatePageAndCheckMessage } from '../../models/admin.methods';

/* global baseUrl*/

describe('Faq', () => {
    before('Pre-conditions', async() => {
        await loginToAdmin();
    });

    it('should open the homepage', async () => {
        await AdminDashboard.leftMenu.pagesTabInLeftMenu().click();
        await AdminDashboard.addNewPage.addNewButtoninPagesTab().click();
        await AdminDashboard.editPostsPage.postTitle().setValue('Test page for maranello bloks page hero with anchor');

        await browser.keys('Enter');
        await browser.keys('/section');
        await browser.keys('Enter');
        await AdminDashboard.constructorBlockspageherowithanchor.addBlockinButtonpageherowithanchor().click();
        await browser.keys('page-hero-with-anchor');
        await browser.pause(2000);

        await AdminDashboard.constructorBlockspageherowithanchor.blockinlistButtonpageherowithanchor().scrollIntoView();
        await AdminDashboard.constructorBlockspageherowithanchor.blockinlistButtonpageherowithanchor().moveTo();
        await AdminDashboard.constructorBlockspageherowithanchor.blockinlistButtonpageherowithanchor().click();
        await updatePageAndCheckMessage();
    });
}); 